...
internal extension String {
    ...
    func matches(_ other: String, _ options: Set<Comparison.Option>, _ locale: Locale?) -> Bool {
    ...
    func begins(with other: String, _ options: Set<Comparison.Option>, _ locale: Locale?) -> Bool {
        let locale = options.contains(.localeSensitive) ? locale : nil
        let compareOptions = CompareOptions(options.compactMap { CompareOptions($0) })
    ...
    func ends(with other: String, _ options: Set<Comparison.Option>, _ locale: Locale?) -> Bool {
        let locale = options.contains(.localeSensitive) ? locale : nil
        let compareOptions = CompareOptions(options.compactMap { CompareOptions($0) })
        guard let range = self.range(of: other, options: compareOptions, range: nil, locale: locale)
            else { return false }
